...
(function () {
  ...
  nav.addEventListener('mouseleave', () => {
    const a = nav.querySelector('a.active') || nav.querySelector('a');
    if (a) moveUnderline(a);
  });
...
(function () {
  ...
  // Fix old broken highlight output such as:
  // accounts[class="hl-num">0]
  // timeout = class="hl-num">3000
  // decimals: class="hl-num">18
  function normalizeCodeText(text) {
    return String(text)
      .replace(/\[\s*class=["']hl-num["']>\s*([0-9]+(?:\.[0-9]+)?)\s*\]/g, '[$1]')
      .replace(/=\s*class=["']hl-num["']>\s*([0-9]+(?:\.[0-9]+)?)/g, '= $1')
      .replace(/:\s*class=["']hl-num["']>\s*([0-9]+(?:\.[0-9]+)?)/g, ': $1')
      .replace(/,\s*class=["']hl-num["']>\s*([0-9]+(?:\.[0-9]+)?)/g, ', $1')
      .replace(/\(\s*class=["']hl-num["']>\s*([0-9]+(?:\.[0-9]+)?)/g, '($1')
      .replace(/\{\s*class=["']hl-num["']>\s*([0-9]+(?:\.[0-9]+)?)/g, '{$1')
      .replace(/\sclass=["']hl-num["']>\s*([0-9]+(?:\.[0-9]+)?)/g, ' $1');
  }
  ...
  function paintPlain(segment) {
    ...
    html = html.replace(
      /\b(const|let|var|function|async|await|return|if|else|for|while|switch|case|break|continue|try|catch|throw|new|class|extends|super|import|from|export|default|undefined|null|true|false)\b/g,
  ...
  function highlightJS(rawInput) {
    const raw = normalizeCodeText(rawInput);
    ...
    while (i < raw.length) {
      ...
      // String literals. This protects URLs such as "kasperia://bridge"
  ...
  function getCopyText(button) {
    ...
    if (pre) {
      ...
      if (code) return normalizeCodeText(code.textContent.trim());
    }

    const parent = button.parentElement;
    const value = parent ? parent.querySelector('.qr-copy-value, code') : null;

    return value ? normalizeCodeText(value.textContent.trim()) : '';
  }
  ...
  document.addEventListener('click', async (event) => {
    const button = event.target.closest(
      '.docs article .copy-btn, .docs article .qr-copy-btn'
    );
